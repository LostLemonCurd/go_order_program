...
func createOrderPDF(order Order, filePath string) error {
	...
	pdf.Ln(10)
	pdf.Cell(40, 10, fmt.Sprintf("Téléphone: %s", order.Customer.PhoneNumber))
	pdf.Ln(10)
	...
	pdf.Ln(10)
	pdf.Cell(40, 10, fmt.Sprintf("Price unitaire: %.2f", order.Product.Price))
	pdf.Ln(10)
	pdf.Cell(40, 10, fmt.Sprintf("Price total: %.2f", order.Price))
	pdf.Ln(10)
